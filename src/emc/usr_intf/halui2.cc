...
\
    ...
    FIELD(hal_bit_t,jog_use_axis_max_speed) /* pin to indicate analog jog speed should be a percentage of the max velocity for that axis */ \
...
static double maxAxisVelocity[EMCMOT_MAX_AXIS+1]={1.0};
...
int halui_hal_init(void)
{
    ...
    *halui_data->ro_direct_value = 0;
    ...
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->jog_use_axis_max_speed), "halui.jog-use-axis-max-speed");
    if (retval < 0) return retval;
...
// axis stuff (selection, homing..)
	...
	if ((floatt != old_halui_data.ajog_analog[axis_num]) || (bit && ajog_speed_changed)) {
	    if (bit) {
                if (new_halui_data.jog_use_axis_max_speed)
                    sendJogCont(axis_num, (maxAxisVelocity[axis_num]) * 60.0 * (new_halui_data.ajog_analog[axis_num]), JOGTELEOP);
                else
