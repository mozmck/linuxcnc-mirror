...
*   
*   Forked from halui.cc to add some features such as analog jogging as
*   a percentange of the max velocity for each axis instead of a set jog
*   speed for all axes.
*
* Authors: Alex Joni, Moses McKnight
...
\
    ...
    FIELD(hal_bit_t,jog_use_joint_max_speed) /* pin to indicate analog jog speed should be a percentage of the max velocity for that joint */ \
...
static double maxAxisVelocity[EMCMOT_MAX_JOINTS+1]={1.0};
...
int halui_hal_init(void)
{
    ...
    *halui_data->so_direct_value = 0;
    ...
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->jog_use_joint_max_speed), "halui.jog-use-joint-max-speed");
    if (retval < 0) return retval;
...
static int iniLoad(const char *filename)
{
    ...
    if(inifile.Find(&maxMaxVelocity, "MAX_VELOCITY", "TRAJ") &&
       inifile.Find(&maxMaxVelocity, "MAX_VELOCITY", "AXIS_0"))
        maxMaxVelocity = 1.0;
    for (int x=0; x<EMCMOT_MAX_JOINTS+1; ++x)
    {
        std::stringstream sstr;
        sstr << "AXIS_" << x;
        if (inifile.Find(&maxAxisVelocity[x], "MAX_VELOCITY", sstr.str().c_str()))
...
// joint stuff (selection, homing..)
	...
	if ((floatt != old_halui_data.jog_analog[joint]) || (bit && jog_speed_changed)) {
	    if (bit) {
	        if (new_halui_data.jog_use_joint_max_speed)
	            sendJogCont(joint,(maxAxisVelocity[joint]) * 60.0 * (new_halui_data.jog_analog[joint]));
	        else
