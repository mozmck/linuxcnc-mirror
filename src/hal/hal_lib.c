...
int hal_stream_write(hal_stream_t *stream, union hal_stream_data *buf) {
    ...
    int in = stream->fifo->in, newin = hal_stream_advance(stream, in);
...
int hal_stream_read(hal_stream_t *stream, union hal_stream_data *buf, unsigned *this_sample) {
    ...
    int out = stream->fifo->out, newout = hal_stream_advance(stream, out);
    int num_pins = stream->fifo->num_pins;
    int stride = num_pins + 1;
...
EXPORT_SYMBOL(hal_stream_create);
EXPORT_SYMBOL(hal_stream_destroy);
EXPORT_SYMBOL(hal_stream_readable);
EXPORT_SYMBOL(hal_stream_depth);
EXPORT_SYMBOL(hal_stream_maxdepth);
EXPORT_SYMBOL(hal_stream_write);
EXPORT_SYMBOL(hal_stream_read);
EXPORT_SYMBOL(hal_stream_attach);
EXPORT_SYMBOL(hal_stream_detach);
EXPORT_SYMBOL(hal_stream_element_count);
EXPORT_SYMBOL(hal_stream_element_type);
EXPORT_SYMBOL(hal_stream_num_overruns);
EXPORT_SYMBOL(hal_stream_num_underruns);
#endif /* rtapi */
